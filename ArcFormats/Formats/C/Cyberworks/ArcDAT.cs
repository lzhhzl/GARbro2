...
namespace GameRes.Formats.Cyberworks
{
    ...
    public class DatOpener : ArchiveFormat
    {
        ...
        internal AImageScheme QueryScheme (string arc_name)
        {
            ...
            var options = Query<BellOptions> (Localization._T ("ArcEncryptedNotice"));
    ...
    internal abstract class IndexReader : IDisposable
    {
        ...
        internal virtual DatPackedEntry ReadEntryInfo ()
        {
            uint id = m_index.ReadUInt32();
            if (id > m_fault_id)
                id = m_fault_id++;
            var entry = new DatPackedEntry { Name = id.ToString ("D6") };
    ...
    internal class ArcIndexReader2 : ArcIndexReader
    {
        ...
        internal override DatPackedEntry ReadEntryInfo ()
        {
            uint id = m_index.ReadUInt32();
            if (id > m_fault_id)
                id = m_fault_id++;
